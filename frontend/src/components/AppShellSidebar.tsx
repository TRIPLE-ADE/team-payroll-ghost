...
export const AppShellSidebar = memo(function AppShellSidebar({
  ...
  return (
    <aside
      ...
      inert={sidebarInert}
      className={cn(
        "grid min-h-0 shrink-0 grid-rows-[auto_minmax(0,1fr)_auto] border-r border-zinc-800/80 bg-zinc-950",
    ...
    >
      <div className="flex shrink-0 items-start justify-between gap-2 border-b border-zinc-800/80 px-4 py-4">
        ...
        <button
          type="button"
          className="-m-1 shrink-0 rounded-md p-2 text-zinc-400 hover:bg-zinc-800 hover:text-zinc-100 md:hidden"
      ...
      <div className="shrink-0 border-t border-zinc-800/80 p-3 text-[10px] leading-snug text-zinc-600">
...
export function AppShellHeader({
  ...
  return (
    <header className="sticky top-0 z-30 flex h-12 shrink-0 items-center gap-3 border-b border-zinc-800/80 bg-zinc-950/95 px-4 backdrop-blur supports-backdrop-filter:bg-zinc-950/80 md:px-6">
      <button
        type="button"
        className="-ml-1 flex h-9 w-9 items-center justify-center rounded-md text-zinc-400 hover:bg-zinc-900 hover:text-zinc-100 md:hidden"
      ...
      <span className="font-mono text-xs text-zinc-500">
        PayGuard · <span className="text-amber-500/80">SANDBOX</span>
      </span>
    </header>
  );
}
